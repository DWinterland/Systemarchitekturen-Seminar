...
\title{Parallelisierung eines NVIDIA GPGPU Client Server Services basierend auf TCP-Sockets}


% author names and affiliations
% use a multiple column layout for up to three different
% affiliations
\author{\IEEEauthorblockN{Til Koke}
\IEEEauthorblockA{Institut f\"ur Betriebssysteme und \\Rechnernetze\\TU Braunschweig \\
M\"uhlenpfordstr.23, 38106 Braunschweig \\
Email: til.koke@tu-bs.de}
\and
\IEEEauthorblockN{Maximilian Wesche}
\IEEEauthorblockA{Institut f\"ur Betriebssysteme und \\Rechnernetze\\TU Braunschweig \\
M\"uhlenpfordstr.23, 38106 Braunschweig \\
Email: maximilian.wesche@tu-bs.de}
\and
\IEEEauthorblockN{David Winterland}
\IEEEauthorblockA{Institut f\"ur Betriebssysteme und \\Rechnernetze\\TU Braunschweig \\
M\"uhlenpfordstr.23, 38106 Braunschweig \\
...
% in the abstract
\begin{abstract}
Die Ausarbeitung befasst sich mit einer verteilten Anwendung für das Auswerten von Stereobildern. Ein vorhandener Algorithmus wird verteilt auf einer Grafikkarte, und auf mehreren Grafikkarten in einem Rechnernetz ausgeführt. Dabei wird eine neue Methode zur Datenübertragung durch TCP-Sockets durchgesetzt. Die Ausführungsgeschwindigkeit der verteilten Anwendung über TCP-Sockets wird mit einer SOAP Kommunikationschnittstelle und einer lokalen Ausführung ohne Rechnernetz verglichen.
\end{abstract}
...
%https://www.genivia.com/doc/soapdoc2.html#tth_sEc11.11
%Algorithmus mit CUDA
%SOAP hat Overhead
%Motiv für C-Sockets: weniger Overhead \\
%(Performance)-Vergleich mit aktuellem Soap Ansatz
 
%\hfill November 24, 2018
...
%\section{Implementierung \& Validierung}
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=3.5in]{Datenformat.pdf} %2.5 ist Voreingestellt
...
Hierbei werden TCP-Stream Sockets verwendet. Als Implementierungsgrundlage werden POSIX Sockets verwendet, die unter den meisten Linux-Distributionen zur Verfügung stehen. % und die Datenpakete mit folgenden Daten gefüllt:  
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=3.5in]{SocketvsWS_bunt_3.pdf} %2.5 ist Voreingestellt
...
%Die Evaluation hat gezeigt, dass die Socketvariante schneller ist als die lokale, sobald die Komplexität der Teilproblemen groß genug ist. Außerdem ist die Serialisierung mit Sockets wesentlich effizienter als mittels GSoap und XML.%
...
Damit hat die Arbeit gezeigt, dass das zusammenschließen mehrerer Grafikkarten für die Berechnung großer Probleme wesentlich schneller sein kann. Gleichzeitig liefert diese Arbeit eine Lösung für die verteilte Berechnung von Stereobilder mit Hilfe von Stream-Sockets die wesentlich schneller ist, als die bestehende Implementierung. % klingt doof, ist aber so. Also eigntlich wusste ich nur nicht, was ich noch schreiben soll.
%Evaluation welches Ergebnis schneller war und warum. 


%TCP Sockets aktuell verwendet, man könnte es auch mit UDP Sockets(Datagram Sockets) machen. Oder eine andere Midlleware bspw. JAVA RMI
...
\bibitem{bl}GSOAP, \url{https://www.genivia.com/doc/soapdoc2.html#tth_sEc11.11}, 6.11.2018.
