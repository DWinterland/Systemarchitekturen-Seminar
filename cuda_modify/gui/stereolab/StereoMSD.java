...
** zur Disparitätenberechnung, der den quadrierte Grauwertdifferenzen als
...
public class StereoMSD extends StereoAlgorithm {

        public StereoMSD(int B, int H, int TauMax, boolean U, int S,boolean F) {
                super(B,H,TauMax,U,S,F);
        }
        
        public String toString() {
                String st =     "Algorithmus:              Quadrat der Diff.\n"+
                        ...
                        for (int j=0;j<spalten;j++) {
                                if ( (i+yu<0) | (i+yo>=zeilen) | (j+xu-tauMax<0) | (j+xo+tauMax>=spalten) ) {
                                        ...
                                        for (int tau=-tauMax;tau<=tauMax;tau++) {
                                                ...
                                                for (int k=xu;k<=xo;k++) {
                                                        for (int l=yu;l<=yo;l++) {
                                                                wert=wert+StereoToolkit.square((StereoToolkit.unsignedByteToInt(lb[i+l][j+k])-StereoToolkit.unsignedByteToInt(rb[i+l][j+k+tau]))*(useF?wc-Math.abs(l)-Math.abs(k):1)); 
                                                        }
                                                }
